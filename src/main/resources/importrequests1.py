...
def store_data_in_db(data):
    """Stores the scraped data while ignoring duplicates"""
    conn = None
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS resultat (
                ...
                UNIQUE KEY unique_resultat (nom_nageur, club, temps)
        ...
        for entry in data:
            try:
                cursor.execute('''
                    ...
                    ON DUPLICATE KEY UPDATE id=id  -- No-op update to ignore duplicates
                ''', (
                    entry.get('Club'),
                    None,
                    entry.get('Naissance'),
                    entry.get('Nation'),
                    entry.get('Nom Nageur'),
                    entry.get('Place'),
                    entry.get('Points'),
                    entry.get('Temps'),
        ...
        conn.commit()
        print(f"Successfully inserted {inserted_count} new records, skipped {duplicate_count} duplicates")
